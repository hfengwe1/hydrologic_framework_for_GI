...
#import os
#os.chdir('C:/School/2nd Paper/Dano')
...
#    P_crit = 1   #threshold to be considered part of a storm.  
                  # days <4mm account for less than 5% of total precip  
...
#    start_time = time.clock()
    ...
    rng = pd.date_range(run_dates[0],run_dates[1], freq='H')
#    prng = pd.period_range(run_dates[0],run_dates[1], freq='H')
    ts = pd.Series(np.zeros(len(rng)), index=rng, name='P')
    df_0 = pd.DataFrame(ts)
    df_P =  df_0.add(df_1, fill_value=0)
    N_hr = len(df_P)
#==============================================================================
#%%Identify storm events 
#==============================================================================
#storm event criteria: minimum gry period of 1 day
#P_crit=4.0
#split_crit=7 #split into two storms if longer than this #days.  
...
#    break_2 = df_P['break_id'][index_break].tolist()
        
    storm_length = [df_P[df_P['storm_id']==x].count()[0] for x in storm_2]
    storm_i = [df_P[df_P['storm_id']==x]['P'].mean() for x in storm_2]
    storm_v = [df_P[df_P['storm_id']==x]['P'].sum() for x in storm_2]
#   create storm dataframe
    max_i = []
#    peak_hr = []

    s_dict={}
    if len(index_storm) == len(index_break):  # this only mean the start is storm or break and the end is the opposite
        N=len(storm_1)-1
        for i in range(N):
            s = df_P['P'][index_storm[i]:index_break[i]]
            s_dict.update({i:s.tolist()})
#            peak_hr.append(len(s[s>=cutoff]))
            max_i.append(max(s))
        # calculate the intensity of the last storm
        s = df_P['P'][index_storm[i]:index_break[i+1]]
        s_dict.update({i:s.tolist()})
        max_i.append(max(s))    
    else:
        N=len(storm_1)
...
#%%Load inputs==============================================================================
def storm_id_adjusted(filename, run_dates, P_crit,P_lag, metric):
#    filename: precipitation data in in/hr
#    run_dates: from xxxx-xx-xx to xxxx-xx-xx
#    P_crit = 1   #threshold to be considered part of a storm.  
                  # days <4mm account for less than 5% of total precip  
