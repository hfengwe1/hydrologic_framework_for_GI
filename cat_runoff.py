...
def sub_runoff(rain,area,imperv_perc,gi_s,gi_d,Sf,w,w_gi,dt,End_T,manning_n):
    ...
    Sf_imp = Sf[1]   # %  
    Sf_gi= Sf[2]
    sub_area= [area*imperv_perc-gi_d,area*( 1-imperv_perc),gi_d]  # area of imperv. and perv. surface
    Area_p=sub_area[1]
...
#    rf_imp[0:len(rf_i)]=rf_i 
    ...
    #### Overflow from GI #########################
    if gi_d >= imperv_perc*area:
        gi_d= imperv_perc*area
...
def Muskingun(inflow, rf_s1,dt,End_T,c0,c1,c2,rain):
    t=dt
    rf_p= rf_s1[0]
    rf_imp= rf_s1[1]
    rf_gi= rf_s1[2]
    Inflow=np.zeros(len(rf_p))
    Inflow[0:len(rf_p)] = inflow+(rf_p + rf_imp + rf_gi)/(dt*3600)  # CFS 
    Outflow = [(1-c1)*Inflow[0]]
    count = 0
    while count < (len(rf_p)-1):
